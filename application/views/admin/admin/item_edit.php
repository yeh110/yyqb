                        	...
                        	<dl class="detail-part">
                            	<dd class="form-row">
                                    ...
                                    <div class="form-value">
                                    	<input type="text" class="form-control" name="item_name" placeholder="您的项目名称1-20个字符" style="width:350px" value="<?=$field['item_name']?>">
                                    </div>
                                </dd>
                                <dd class="form-row">
                                	<label>绑定域名：</label>
                                    <div class="form-value">
                                    	<textarea class="form-control" rows="6" name="domains" style="width:350px"  placeholder="您要绑定的域名，一行一个，如：img.youdomain.com"><?=$this->Item_model->json2domainstr($field['domains'])?></textarea>
                                    </div>
                                </dd>
                                <dd class="form-row">
                                	<label></label>
...
$(function () {
    $('#myfrom').submit(function(){
        if($('input[name="item_name"]').val()==''){
            ...
            yh_ajax_post('<?=site_url('item/editAjax')?>', $(this).serialize(), function(data){
                if(data.errNum===0){
