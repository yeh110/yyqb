...
KindEditor.plugin('filemanager', function(K) {
	...
	self.plugin.filemanagerDialog = function(options) {
		var width = K.undef(options.width, 650),
			height = K.undef(options.height, 530),
		...
		function createView(result) {
			...
			K(".xl_span").click(function () {
				...
				$.post(K.addParam(uploadJson, 'dir=delete')+'&path='+$this.attr("data-url"),function(res){
                    res == '10028' ? $this.parent().remove() : alert(lang.deleteError);
                    if (K(".ke-plugin-filemanager-body").children().length < 1) { K(".ke-plugin-filemanager-body").html(lang.noImg) }
                })
            })
		}
